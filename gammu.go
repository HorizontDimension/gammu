...
func NewStateMachineFromStrConfig(Config Config) (*StateMachine, error) {
	...
	var cfg *C.GSM_Config
	
	cfgDevice := C.CString(Config.Device)
	defer C.free(unsafe.Pointer(cfgDevice))
	cfgConn := C.CString(Config.Connection)
	defer C.free(unsafe.Pointer(cfgConn))

	cfg = C.GSM_GetConfig(sm.g, 0)

	cfg.Device = cfgDevice
	cfg.Connection = cfgConn
	
	
	C.GSM_SetConfigNum(sm.g, 1)

	//if e := C.GSM_ReadConfig(cfg, C.GSM_GetConfig(sm.g, 0), 0); e != C.ERR_NONE {
	//	sm.free()
